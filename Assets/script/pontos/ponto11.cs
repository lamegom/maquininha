...
public class ponto11 : MonoBehaviour {
	...
	void OnTriggerStay(Collider obj){
		if( rodarRoleta5.tempo == 0 && teste){
			ponto = obj.GetComponent<Renderer>().material.mainTexture + "";
			teste = false;
		...
		if(obj.gameObject.GetComponent<Renderer>().material.mainTexture == null)
			obj.gameObject.GetComponent<Renderer>().material.mainTexture = variaveis.imagem[salvaArmazena];
		
		if(liberaSelecao){
			obj.gameObject.GetComponent<Renderer>().material.mainTexture = variaveis.imagemGanho[salvaArmazena];
		}
