...
public class l9 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 9){
			chamaLine();
		}

		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 9){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto6.liberaSelecao = ponto12.liberaSelecao = ponto13.liberaSelecao = ponto14.liberaSelecao = ponto10.liberaSelecao = true;
										...
										if(j == 1 && figurasPorLinhas != 4){
											...
											if(pontuacao[i] == pontuacao[k] && pontuacao[i] == pontuacao[m] && pontuacao[i] != "slot04c (UnityEngine.Texture2D)" && figurasPorLinhas == 0 ){
												...
												if(i == 0 && k == 1 && m == 2){
													...
													chamaLine(); // chama a linha na tel
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(2.0F,Screen.height/2.0f,20,20),"9");
		}
