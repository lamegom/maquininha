...
public class l6 : MonoBehaviour {
	...
	public Color c1 = new Color(1,0,0.5f);
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 6){
			chamaLine();
		}

		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 6){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto1.liberaSelecao = ponto2.liberaSelecao = ponto8.liberaSelecao = ponto14.liberaSelecao = ponto15.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(0, new Vector3(-4, 2.2f,-5) );
		lineRenderer.SetPosition(1,new Vector3 (-3, 2.2f,-5));
		lineRenderer.SetPosition(2,new Vector3 (-1.5f,2.2f,-5));
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(2.0F,Screen.height/3.65f,20,20),"6");
		}
