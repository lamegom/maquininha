...
public class l14 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 14){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 14){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto15.liberaSelecao = ponto9.liberaSelecao = ponto3.liberaSelecao = ponto2.liberaSelecao = ponto1.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(3,new Vector3 (0, 2.3f,-5) );
		lineRenderer.SetPosition(4, new Vector3(-1.5f, 2.3f,-5) );
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(Screen.width-25,Screen.height/1.60f,25,20),"14");
		}
