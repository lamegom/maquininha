...
public class l12 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 12){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 12){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto5.liberaSelecao = ponto4.liberaSelecao = ponto3.liberaSelecao = ponto7.liberaSelecao = ponto11.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(0, new Vector3(4, 2.3f,-5) );
		lineRenderer.SetPosition(1,new Vector3 (3, 2.3f,-5));
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(Screen.width-25,Screen.height/3.75f,25,20),"12");
		}
