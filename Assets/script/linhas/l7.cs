...
public class l7 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 7){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 7){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto11.liberaSelecao = ponto12.liberaSelecao = ponto8.liberaSelecao = ponto4.liberaSelecao = ponto5.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(0, new Vector3(-4, 0.2f,-5) );
		lineRenderer.SetPosition(1,new Vector3 (-3, 0.2f,-5));
		lineRenderer.SetPosition(2,new Vector3 (-1.5f,0.2f,-5));
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(2.0F,Screen.height/1.65f,20,20),"7");
		}
