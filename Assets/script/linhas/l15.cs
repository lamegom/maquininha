...
public class l15 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 15){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 15){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto11.liberaSelecao = ponto9.liberaSelecao = ponto13.liberaSelecao = ponto12.liberaSelecao = ponto11.liberaSelecao = true;
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(Screen.width-25,Screen.height/3.50f,25,20),"15");
		}
