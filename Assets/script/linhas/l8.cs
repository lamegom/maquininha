...
public class l8 : MonoBehaviour {
	...
	public Color c1 = new Color(0.5f,0.5f,0);
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 8){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 8){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto6.liberaSelecao = ponto2.liberaSelecao = ponto3.liberaSelecao = ponto4.liberaSelecao = ponto10.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(0, new Vector3(-4, 1.15f,-5) );
		lineRenderer.SetPosition(1,new Vector3 (-3, 1.15f,-5));
		lineRenderer.SetPosition(2,new Vector3 (-1.5f,2.4f,-5));
		lineRenderer.SetPosition(3,new Vector3 (0, 2.4f,-5) );
		lineRenderer.SetPosition(4, new Vector3(1.5f, 2.4f,-5) );
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(2.0F,Screen.height/2.2f,20,20),"8");
		}
