...
public class l19 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 19){
			chamaLine();
		}

		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 19){
			if(teste){
				string[] pontuacao =  new string[5];
				pontuacao[0] = ponto15.ponto;
				pontuacao[1] = ponto14.ponto;
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto15.liberaSelecao = ponto14.liberaSelecao = ponto13.liberaSelecao = ponto12.liberaSelecao = ponto6.liberaSelecao = true;
										...
										if(j == 0){
											...
											if(pontuacao[i] == pontuacao[k] && pontuacao[i]== pontuacao[m] && pontuacao[i]== pontuacao[n] && figurasPorLinhas == 0){
												...
												if(i == 0 && k == 1 && m == 2 && n == 3 ){
													...
													if(i == 0) 			{ ponto15.liberaSelecao = true;}										
													if(i == 1 || k == 1){ ponto14.liberaSelecao = true;}						
										...
										if(j == 1 && figurasPorLinhas != 4){
											...
											//direita
												...
												if(k == 2 && m == 3 && n == 4 || k == 1 && m == 2 && n == 3){
													...
													ganhoLinhas.premio();
													// ativa as figuras da linha
													if(k == 1 )			{ ponto14.liberaSelecao = true;}						
													if(k == 2 || m == 2){ ponto13.liberaSelecao = true;}
											...
											//034
												...
												// ativa as figuras da linha
												if(i == 0) 			{ ponto15.liberaSelecao = true;}		
												if(i == 1 ){ ponto14.liberaSelecao = true;}						
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(0, new Vector3(4, 0.3f,-5) );
		lineRenderer.SetPosition(1,new Vector3 (3, 0.3f,-5));
		lineRenderer.SetPosition(2,new Vector3 (1.5f,0.3f,-5));
		lineRenderer.SetPosition(3,new Vector3 (0, 0.3f,-5) );
		lineRenderer.SetPosition(4, new Vector3(-1.5f, 0.3f,-5) );
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(Screen.width-25,Screen.height/1.70f,25,20),"19");
		}
