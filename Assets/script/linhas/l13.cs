...
public class l13 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 13){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 13){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto15.liberaSelecao = ponto14.liberaSelecao = ponto13.liberaSelecao = ponto7.liberaSelecao = ponto1.liberaSelecao = true;
										...
										if(j == 0){
											...
											if(pontuacao[i] == pontuacao[k] && pontuacao[i]== pontuacao[m] && pontuacao[i]== pontuacao[n] && figurasPorLinhas == 0){
												...
												if(i == 0 && k == 1 && m == 2 && n == 3 ){
													...
													if(n == 4)			{ ponto1.liberaSelecao = true;}
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(1,new Vector3 (3, 0,-5));
		lineRenderer.SetPosition(2,new Vector3 (1.5f,0,-5));
		lineRenderer.SetPosition(3,new Vector3 (0, 0,-5) );
		lineRenderer.SetPosition(4, new Vector3(-1.5f, 1,-5) );
		lineRenderer.SetPosition(5, new Vector3(-3, 2.3f,-5) );
		lineRenderer.SetPosition(6, new Vector3(-4, 2.3f,-5) );
		TeclaLinha = true;
	}
	bool TeclaLinha;
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(Screen.width-25,Screen.height/1.55f,25,20),"13");
		}
