...
public class l17 : MonoBehaviour {
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 17){
			chamaLine();
		}

		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 17){
			if(teste){
				...
				if(pontuacao[0] == pontuacao[1] && pontuacao[0] == pontuacao[2] && pontuacao[0] == pontuacao[3] && pontuacao[0] == pontuacao[4]){
					...
					ganhoLinhas.premio();
					ponto10.liberaSelecao = ponto14.liberaSelecao = ponto13.liberaSelecao = ponto12.liberaSelecao = ponto11.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(0, new Vector3(4, 0.9f,-5) );
		lineRenderer.SetPosition(1,new Vector3 (3, 0.9f,-5));
	...
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(Screen.width-25,Screen.height/2.0f,25,20),"17");
		}
