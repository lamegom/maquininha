...
public class l5 : MonoBehaviour {
	...
	public Color c1 = new Color(0.5f,0,1);
	...
	void Update () {
		...
		if(layout.mostratecla && layout.tecla >= 5){
			chamaLine();
		}


		if(!parada5.teste && !ponto5.teste && !ponto10.teste && !ponto15.teste && layout.tecla >= 5){
			if(teste){
				...
				//linha 5
				if(ponto1.ponto == ponto7.ponto && ponto1.ponto == ponto13.ponto && ponto1.ponto == ponto9.ponto && ponto1.ponto == ponto5.ponto){
					chamaLine();
					ponto1.liberaSelecao = ponto7.liberaSelecao = ponto13.liberaSelecao = ponto9.liberaSelecao = ponto5.liberaSelecao = true;
	...
	void chamaLine(){
		...
		lineRenderer.SetPosition(2,new Vector3 (0,0,-5));
		lineRenderer.SetPosition(3,new Vector3 (3,2.5f,-5) );
		lineRenderer.SetPosition(4, new Vector3(4,2.5f,-5) );
		TeclaLinha = true;
	}
	bool TeclaLinha;
	void OnGUI(){
		if(TeclaLinha){
			GUI.Label(new Rect(2.0F,Screen.height/4.0f,20,20),"5");
		}
