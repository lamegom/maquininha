...
public class layout : MonoBehaviour {
	...
	void Update () {
		...
		if(rodarRoleta5.tempo == 0){
			...
			if( (Input.GetButton("Y") || keyboardText=="Y") && ganhoLinhas.bonus[5] == 0 && bancoDados.premio == 0){
				keyboardText = "";
			...
			//botão aposta
			if( ( Input.GetButtonDown("G") || keyboardText == "G") && !auto && ganhoLinhas.bonus[0] == 0 && ganhoLinhas.bonus[1] == 0 && ganhoLinhas.bonus[2] == 0 && ganhoLinhas.bonus[3] == 0 && ganhoLinhas.bonus[4] == 0 && ganhoLinhas.bonus[5] == 0 && bancoDados.premio == 0 && ganhoLinhas.bonus[6] == 0 ){
				keyboardText="";
				ganhoLinhas.valendo = false;
				bancoDados.aposta++;
			...
			// botao jogar zera todas as variaveis da roleta
				...
				if(cont > 0){
					...
					if(Input.GetButton("B") || keyboardText=="B"){
					...
					if(ganhoLinhas.bonus[5] > 0){
						...
						print("ganhoLinhas.rodadaGratis: " + ganhoLinhas.rodadaGratis);

						if(ganhoLinhas.rodadaGratis){
							ganhoLinhas.rodadaGratis = false;
							GetComponent<AudioSource>().PlayOneShot(bonus[7]);
						}
							jogaMoedas.ativa = false;
							roletaImag1.delay = 2;
							rodarRoleta1.tempo = -80;
		...
		if( Input.GetButtonDown("P") || Input.GetButton("P") || keyboardText == "P"){
			keyboardText = "";
	...
	void OnGUI(){
        ...
        if (GUI.Button(new Rect (30,Screen.height / 2 - 100,150,130*aspectRatio), icon) )
		{
	 		 keyboardText = "Y";
        }
	...
	//	}
		...
		if(carregandoBool) {
			...
			//Rect c = new Rect(Screen.width/5.517239F,Screen.height/60,Screen.width/5,Screen.height/12);
			GUI.DrawTexture(new Rect(Screen.width/120,Screen.height/85.714f,Screen.width/12*aspectRatio,Screen.height/15*aspectRatio),credito);
			Rect c = new Rect(Screen.width/5.517239F,Screen.height/60,Screen.width/5,Screen.height/12*aspectRatio);
			GUI.DrawTexture(c,balao);
			 if (GUI.Button(c , "", new GUIStyle())){
    			if (c.Contains(Event.current.mousePosition)) {
					keyboardText = "P";
				}
			}

			GUI.Label(new Rect(Screen.width/5.161291F,Screen.height/35.29458f,250,200*aspectRatio),"" + bancoDados.carregaCredito(),estilo);
			...
			GUI.DrawTexture(new Rect(Screen.width/2.580645f,Screen.height/85.714f,Screen.width/12*aspectRatio,Screen.height/15*aspectRatio),premio);
			GUI.DrawTexture(new Rect(Screen.width/1.8F,Screen.height/60,Screen.width/5,Screen.height/12*aspectRatio),balao);
			GUI.Label(new Rect(Screen.width/1.73913F,Screen.height/35.29458f,250,200*aspectRatio),"" + bancoDados.premio,estilo);
			...
			//Rect a = new Rect(Screen.width/1.134752f,Screen.height/60,Screen.width - (Screen.width- 90),Screen.height/12);
			GUI.DrawTexture(new Rect(Screen.width/1.311475f,Screen.height/85.714f,Screen.width/12*aspectRatio,Screen.height/15*aspectRatio),aposta);
			//Rect a = new Rect(Screen.width/1.134752f,Screen.height/60,Screen.width - (Screen.width- 90),31*aspectRatio);
			Rect a = new Rect(Screen.width/1.134752f,Screen.height/60,Screen.width/5,Screen.height/12*aspectRatio);


			GUI.DrawTexture(a ,balaoAposta);
			
			 if (GUI.Button(a , "", new GUIStyle())){

	 			if (a.Contains(Event.current.mousePosition)) {
					keyboardText = "G";
				}
			}
			GUI.Label(new Rect(Screen.width/1.111111f,Screen.height/35.29458f,250,200*aspectRatio),"" + (tecla*bancoDados.aposta*bancoDados.valorAposta),estilo);
			
			//aposta e morcego
			//GUI.DrawTexture(new Rect(Screen.width/1.311476f,Screen.height/13.33337f,morcego.width/1.4f,morcego.height/1.2f),morcego);
			GUI.DrawTexture(new Rect(Screen.width/1.311476f,Screen.height/13.33337f*aspectRatio,morcego.width/1.4f,morcego.height/1.2f),morcego);
			GUI.Label(new Rect(Screen.width/1.230779f,Screen.height/12.00007f*aspectRatio,250,150*aspectRatio),"" + bancoDados.aposta, estilo2);
			//ultimo valor pago
			GUI.Label(new Rect(Screen.width/20,Screen.height/8.0007f,200,25),"Pago: " + bancoDados.ganho, estilo5);
