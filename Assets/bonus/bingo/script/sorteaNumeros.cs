...
public class sorteaNumeros : MonoBehaviour {
	...
	void Update () {
		...
		if(Input.GetKey(KeyCode.A)){
			pausado = false;
			Time.timeScale = 0.0f;
			ajuda.GetComponent<Renderer>().enabled = true;
		}
		else{
			Time.timeScale = 1.0f;
			ajuda.GetComponent<Renderer>().enabled = false;
			pausado = true;
		}
	...
	void fala(){
		...
		if(SomaGanho == 12 && SalvaPontos == 0){
			SalvaPontos++;
			GetComponent<AudioSource>().PlayOneShot(pontos[Random.Range(0,2)], 1.0f);
		}
		if(SomaGanho == 13 && SalvaPontos <= 1){
			SalvaPontos = 2;
			GetComponent<AudioSource>().PlayOneShot(pontos[Random.Range(2,5)], 1.0f);
		}
	}
